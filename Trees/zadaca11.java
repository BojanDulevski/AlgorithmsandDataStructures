...
class Node {
    ...
    int rightCount; // број на јазли во десното поддрво
    Node(int val) {
        this.val = val;
        this.leftCount = 0;
        this.rightCount = 0;
...
public class Main {
    ...
    static Node insert(Node root, int val) {
        ...
        if (val < root.val) {
            ...
            root.rightCount++; // зголеми број на јазли во десното поддрво
        }
        return root;
    }

    static int countGreater(Node root, int val) {
        int count = 0;
        Node cur = root;
        while (cur != null) {
            if (val >= cur.val) {
                ...
                count += 1 + (cur.rightCount); // самиот јазел + сите десни
                cur = cur.left;
            }
        }
        return count;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N + Q; i++) {
            ...
            if (parts[0].equals("insert")) {
                ...
                int val = Integer.parseInt(parts[1]);
                System.out.println(countGreater(root, val));
            }
        }
    }
}
